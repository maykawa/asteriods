...
function createAsteriodField(number) {
    asteriods = [];
...
function draw() {
    ...
    background(0);
    if (!gamePaused) {
        asteriods.forEach(updateAsteriods);
        updateLasers();
        ship.update();
        ufo.update();
    }
    asteriods.forEach(drawAsteriods);
